...
describe User do
  ...
  describe 'ユーザー新規登録' do
    context '新規登録がうまくいくとき' do
      ...
      it "passwordが英数字混合の6文字以上のであれば登録できる" do
        @user.password = "00000a"
        @user.password_confirmation = "00000a"
        expect(@user).to be_valid
      end

      it "first_nameが全角で入力されると登録できる" do
        @user.first_name = "あ"
        expect(@user).to be_valid
      end

      it "last_nameが全角で入力されると登録できる" do
        @user.last_name = "あ"
        expect(@user).to be_valid
      end

      it "first_name_kanaが全角カタカナで入力されると登録できる" do
        @user.first_name_kana = "ア"
        expect(@user).to be_valid
      end

      it "last_name_kanaが全角カタカナで入力されると登録できる" do
